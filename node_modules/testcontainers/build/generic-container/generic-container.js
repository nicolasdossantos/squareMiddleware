...
class GenericContainer {
    ...
    async reuseContainer(client, container) {
        ...
        if (!inspectResult.State.Running) {
            ...
            inspectResult = (await (0, inspect_container_util_ports_exposed_1.inspectContainerUntilPortsExposed)(() => client.container.inspect(container), this.exposedPorts, container.id)).inspectResult;
        }
    ...
    async startContainer(client) {
        ...
        const { inspectResult, mappedInspectResult } = await (0, inspect_container_util_ports_exposed_1.inspectContainerUntilPortsExposed)(() => client.container.inspect(container), this.exposedPorts, container.id);
        const boundPorts = bound_ports_1.BoundPorts.fromInspectResult(client.info.containerRuntime.hostIps, mappedInspectResult).filter(this.exposedPorts);
        if (this.startupTimeoutMs !== undefined) {
            this.waitStrategy.withStartupTimeout(this.startupTimeoutMs);
        }
    ...
    withExposedPorts(...ports) {
        ...
        for (const exposedPort of ports) {
            exposedPorts[`${(0, port_1.getContainerPort)(exposedPort).toString()}/tcp`] = {};
        }
