...
Azure's `azure/webapps-deploy@v2` action has a built-in rule that **excludes `node_modules`** from deployment because:
...
Our workflow was copying pre-built `node_modules` to deployment folder, triggering this exclusion rule → app started without dependencies → crash.
